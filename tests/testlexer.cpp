...
TEST_CASE("LexerInteger", "[Literals]")
{
    ...
    SECTION( "single int literal" )
    {
        auto ss = std::stringstream("54321");
        lexer.scan(ss);
    ...
    SECTION( "two int literals separated by a space" )
    {
        auto ss = std::stringstream("54 321");
        lexer.scan(ss);
...
TEST_CASE("LexerFloat", "[Literals]")
{
    ...
    SECTION("two float literals separated by a space")
    {
        auto ss = std::stringstream("5.4 3.21");
        lexer.scan(ss);
        REQUIRE(tokens.size() == 3);
...
TEST_CASE("LexerString", "[Literals]")
{
    ...
    SECTION("string 'abcdef")
    {
        auto ss = std::stringstream("\"abcdef\"");
        lexer.scan(ss);
        REQUIRE(tokens.size() == 2);
    ...
    SECTION("string 'abc def gh\"i'")
    {
        const char* multiStr = R"("abc" "def" "gh\"i")";
        auto ss = std::stringstream(multiStr);
        lexer.scan(ss);
...
TEST_CASE("LexerBinaryOperators", "[Operators]")
{
    ...
    {
        auto ss = std::stringstream("1+1.0");
        lexer.scan(ss);
        REQUIRE(tokens.size() == 4);
    ...
    SECTION("paired binary '/='")
    {
        auto ss = std::stringstream(R"(15.2 /="string")");
        lexer.scan(ss);
...
TEST_CASE("LexerUnaryOperators", "[Operators]")
{
    ...
    SECTION("operators separated by spaces")
    {
    ...
    SECTION("mix identifier and operators")
    {
        auto ss = std::stringstream(R"(str+="a"+"b")");
        lexer.scan(ss);
...
TEST_CASE("LexerFunctions", "[functions]")
{
    Lexer lexer;
    const auto& tokens = lexer.getTokens();
    SECTION("main function returns 0")
    {
const char* mainFunctionStr = R"(
    int main(int argc, const char* argv[])
    {
        ...
        auto ss = std::stringstream(mainFunctionStr);
        lexer.scan(ss);

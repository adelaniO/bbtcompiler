...
TEST_CASE("parse_expression", "[expression][parser]")
{
    std::string test{"binary_expression"};
    SECTION(test)
    {
        const auto testFileName = std::string(BBT_SOURCE_DIR_STRING) + "/tests/testData/parser/" + test + ".txt";
        const auto expectedFileName = std::string(BBT_SOURCE_DIR_STRING) + "/tests/testData/parser/" + test + ".json";
        REQUIRE(fs::exists(testFileName));
        REQUIRE(fs::exists(expectedFileName));
        Parser parser;
        const json& resultjson = parser.parse(std::ifstream(testFileName));
        const json& expectedjson = json::parse(std::ifstream(expectedFileName));
        CHECK(resultjson.dump(2) == expectedjson.dump(2));
    }
    ...
    test = "assignment_expression_with_init";
    SECTION(test)
    {
        const auto testFileName = std::string(BBT_SOURCE_DIR_STRING) + "/tests/testData/parser/" + test + ".txt";
        const auto expectedFileName = std::string(BBT_SOURCE_DIR_STRING) + "/tests/testData/parser/" + test + ".json";
...
TEST_CASE("parse_print_statement", "[print][statement][parser]")
{
    std::string test{"print_statement"};
    SECTION(test)
    {
        const auto testFileName = std::string(BBT_SOURCE_DIR_STRING) + "/tests/testData/parser/" + test + ".txt";
        const auto expectedFileName = std::string(BBT_SOURCE_DIR_STRING) + "/tests/testData/parser/" + test + ".json";
...
TEST_CASE("parse_compound_statement", "[compound][statement][parser]")
{
    std::string test{"compound_statement"};
    SECTION(test)
    {
        const auto testFileName = std::string(BBT_SOURCE_DIR_STRING) + "/tests/testData/parser/" + test + ".txt";
        const auto expectedFileName = std::string(BBT_SOURCE_DIR_STRING) + "/tests/testData/parser/" + test + ".json";
